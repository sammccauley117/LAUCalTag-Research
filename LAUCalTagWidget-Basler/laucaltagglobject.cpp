...
LAUCalTagGLObject::LAUCalTagGLObject(QObject *parent) : QObject(parent), isValidFlag(false)
{
    ...
    // SAM'S CODE -- CHANGED MINBOXCOUNT TO 2, MINRAGIONAREA TO MAXREGIONAREA / 20, AND FLIPCALTAGSFLAG TO TRUE
    minBoxCount     = 2;                    // 2X2 PIXELS TIMES 8X8 BOX SIZE
    maxRegionArea   = (640 * 480) / 16;     // NEEDS IMAGE SIZE AND BYTES PER PIXEL
    minRegionArea   = maxRegionArea / 20;
...
void LAUCalTagGLObject::processGL(QOpenGLTexture *videoTexture, QOpenGLFramebufferObject *outputFBO)
{
    if (isValid()) {
        ...
        memoryObject[0].save(QString("C:/Users/Sam/Documents/Git/UK-Research/LAUCalTagWidget-Basler/tmp/memoryA.tif"));
        ...
        memoryObject[1].save(QString("C:/Users/Sam/Documents/Git/UK-Research/LAUCalTagWidget-Basler/tmp/memoryB.tif"));
...
#endif
{
    ...
    // SAM'S CODE
    cv::Mat dirImage(dbObj.height(), dbObj.width(), CV_8UC3, (unsigned char *)dbObj.constPointer(), dbObj.step()); // IMG USED TO SHOW DIRECTION VECTORS
    // END SAM'S CODE
#ifdef QT_DEBUG
    ...
    for (unsigned int n = 0; n < coordinates.size(); n++) {
        for (unsigned int m = 0; m < coordinates[n].size(); m++) {
            if (qIsNaN(coordinates[n][m].x * coordinates[n][m].y) == false) {
                cv::circle(dbImage, quads[n][m], 2, cv::Scalar(0, 0, 255), -1);
            }
        }
    }
    //cv::imshow("", dbImage);
...
/*******************************SAM'S CODE**********************************/
/***************************************************************************/
void LAUCalTagGLObject::findDirection(cv::Mat img, QList<Pairing> pairings)
{
    // DECLARATIONS/INITIALIZATIONS
    cv::Point2f origin; origin.x = NAN; origin.y = NAN; // POINT: (0,0)
    cv::Point2f xPoint; xPoint.x = NAN; xPoint.y = NAN; // POINT: (1,0)
    cv::Point2f yPoint; yPoint.x = NAN; yPoint.y = NAN; // POINT: (0,1)
    // cv::Point2f zPoint; zPoint.x = NAN; zPoint.y = NAN; // POINT: (0,1)

    cv::Point2f xVec;   xVec.x   = NAN; xVec.y   = NAN; // VECTOR: X DIRECTION
    cv::Point2f yVec;   yVec.x   = NAN; yVec.y   = NAN; // VECTOR: Y DIRECTION

    // LOOP THROUGH PAIRINGS TO SEE IF (0,0), (1,0), OR (0,1) WERE CAPTURED
    QList<LAUCalTagGLObject::Pairing>::iterator i;
    for (i = pairings.begin(); i != pairings.end(); ++i){
        cv::circle(img, cv::Point2f(i->cr.x(), i->cr.y()), 2, cv::Scalar(0, 0, 255), -1); // APPLY A RED DOT TO EVERY CAPTURED COORDINATE
        if(i->xy.x() == 0 && i->xy.y() == 0){ // XY: (0,0) WAS CAPTURED, SET THE ORIGIN PIXEL LOCATION
        ...
        if(i->xy.x() == 1 && i->xy.y() == 0){ // XY: (1,0) WAS CAPTURED, SET THE XPOINT PIXEL LOCATION
            xPoint = cv::Point2f(i->cr.x(), i->cr.y());
        }
        if(i->xy.x() == 0 && i->xy.y() == 1){ // XY: (0,1) WAS CAPTURED, SET THE YPOINT PIXEL LOCATION
            yPoint = cv::Point2f(i->cr.x(), i->cr.y());
        }
    }

    // CALCULATE AND DRAW DIRECTION VECTOR
    if(!qIsNaN(origin.x) && !qIsNaN(xPoint.x)){ // ORIGIN AND XPOINT WERE BOTH CAPTURED, DISPLAY AN ARROW FOR THE X DIRECTION AND CALCULATE X VECTOR
        cv::arrowedLine(img, origin, xPoint, cv::Scalar(255, 255, 0), 2);
        xVec = cv::Point2f(xPoint.x - origin.x, origin.y - xPoint.y);
        // qDebug() << "pO(" << origin.x << "," << origin.y << ") " << "pX(" << xPoint.x << "," << xPoint.y << ") " << "V(" << xVec.x << "," << xVec.y << ") ";
    }
    if(!qIsNaN(origin.x) && !qIsNaN(yPoint.x)){ // ORIGIN AND YPOINT WERE BOTH CAPTURED, DISPLAY AN ARROW FOR THE Y DIRECTION AND CALCULATE Y VECTOR
        cv::arrowedLine(img, origin, yPoint, cv::Scalar(0, 255, 255), 2);
        yVec = cv::Point2f(yPoint.x - origin.x, origin.y - yPoint.y);
    }

    // if(!qIsNaN(xVec.x) && !qIsNaN(yVec.x)){ // ESTIMATES Z-AXIS
    //     cv::Point2f zPoint = cv::Point2f(xVec.x * yVec.y, yVec.x * xVec.y);
    //     zPoint = cv::Point2f(origin.x + zPoint.y, origin.y - zPoint.x);
    //     cv::arrowedLine(img, origin, zPoint, cv::Scalar(255, 0, 255), 2);
    // }

    // LOG TO FILE
    QFile file(QString("direction.txt"));
    if (file.open(QIODevice::Append | QIODevice::Text)) {
        // ORIGIN.X, ORIGIN.Y, XPOINT.X, XPOINT.Y, YPOINT.X, YPOINT.Y, XVEC.X, XVEC.Y, YVEC.X, YVEC.Y
        QString toPrint = QString("");
        QTextStream stream(&file);

        // THE TIME ELAPSED
        qint64 nanoSec = timer.nsecsElapsed();
        qint64 sec     = nanoSec / 1000000000;
        qint64 secR    = nanoSec % 1000000000;
        toPrint += QString("%1.%2, ").arg(sec).arg(secR);

        // THE ACTUAL PIXEL COORDINATES OF ORIGIN, XPOINT, AND YPOINT
        toPrint += QString("%1, %2, ").arg(origin.x).arg(origin.y);
        toPrint += QString("%1, %2, ").arg(xPoint.x).arg(xPoint.y);
        toPrint += QString("%1, %2, ").arg(yPoint.x).arg(yPoint.y);

        // THE DIRECTION VECTORS
        toPrint += QString("%1, %2, ").arg(xVec.x).arg(xVec.y);
        toPrint += QString("%1, %2").arg(yVec.x).arg(yVec.y);
